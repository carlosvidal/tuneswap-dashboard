...
class TuneSwapDashboard {
    ...
    updateServerStatus(isOnline, healthData = null) {
        ...
        if (isOnline) {
            ...
            if (healthData) {
                statusText.textContent = `Online (${healthData.events || 0} events, ${healthData.users || 0} users)`;
            } else {
    ...
    displayAnalytics(data) {
        // Update stats
        document.getElementById('total-users').textContent = this.formatNumber(data.totalUsers || 0);
        document.getElementById('total-conversions').textContent = this.formatNumber(data.totalConversions || 0);
        document.getElementById('daily-conversions').textContent = this.formatNumber(data.dailyConversions || 0);
    ...
    displayTopSongs(songs) {
        ...
        container.innerHTML = songs.map(song => `
            <div class="list-item">
                ...
                <div class="conversion-count">${this.formatNumber(song.conversions)}</div>
            </div>
        `).join('');
    }
        ...
        container.innerHTML = websites.map(site => `
            <div class="list-item">
                <div class="song-info">
                    ...
                    <span class="category">${(site.category || 'other').replace('_', ' ')}</span>
                </div>
                <div class="conversion-count">${this.formatNumber(site.conversions)}</div>
            </div>
        `).join('');
    }
    ...
    displayRealtimeActivity(activities) {
        ...
        container.innerHTML = activities.map(activity => `
            <div class="list-item">
                <div class="song-info">
                    <div class="song-title">${this.escapeHtml(activity.song)} - ${this.escapeHtml(activity.artist)}</div>
                    <div class="song-artist">from ${activity.website}</div>
                </div>
                <div style="color: #666; font-size: 0.9rem;">${activity.time}</div>
            </div>
        `).join('');
    }
    ...
    async exportData() {
        try {
            ...
            link.setAttribute("download", `tuneswap-analytics-${new Date().toISOString().split('T')[0]}.csv`);
